...
- **Fields:**
  ...
  - `records` (`array`): Product-level entries containing the latest visible prices as well as their version history.
    ...
    - Version metadata:
      ...
      - `latest_price_change_version` (`string`): Newest version where any price or related metadata (`price_*`, `valid_until`, `reference_prices`, `reference_prices_secondary`) changed.
      - `version_history` (`array`): Snapshots ordered from newest to oldest, deduplicated so that only versions where prices/meta changed are included. Each snapshot contains `version`, `price_wholesale`, `price_with_margin`, `price_retail`, `valid_until`, and optional `reference_prices` maps.
...
```json
{
  ...
  "latest_version": "1.10",
  "latest_price_change_version": "1.9",
  "version_history": [
    {
      "version": "1.9",
      "price_wholesale": 11.16,
