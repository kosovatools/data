...
const WORKSPACE_ROOT = process.env.WORKSPACE_ROOT || process.env.INIT_CWD || process.cwd()
const SOURCE = process.env.CUSTOMS_DATA_SOURCE_URL || ""
const DATA_PATH = path.resolve(WORKSPACE_ROOT, "data/customs/tarrifs.json")
...
async function fetchTariffs({
    ...
    console.log(`Saved ${parsed.length} records to ${path.relative(WORKSPACE_ROOT, outputPath)}`)
